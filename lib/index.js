...
function plugin(opts) {
  ...
  opts.first = opts.first || 'tags/:tag',
  ...
  opts.slug = "slug" in opts ? slug : {mode: 'rfc3986'};
  ...
  return function(files, metalsmith, done) {
    ...
    let topLevelBucketName = opts.topLevelBucket;
    let topLevelBucket = (metadata[topLevelBucketName] = metadata[topLevelBucketName] || {});
    ...
    for (var tag in tagList) {
      ...
      if (topLevelBucket[opts.metadatakey][tag]) {
        console.log("Warning! metadata in ",
                    topLevelBucketName + "[" + opts.metadatakey + "]" + "[" + tag + "]",
      ...
      selection.first = getFilePath(opts.first, {tag: tag});
      selection.layout = opts.layout;
      selection.pageMetadata = opts.pageMetadata;
    };

    // update metadata
    metadata[topLevelBucketName] = topLevelBucket;
